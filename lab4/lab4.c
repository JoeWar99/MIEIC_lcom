...
int(mouse_test_packet)(uint32_t cnt)
{
  ...
  extern fila *fila_transmitter;
  extern fila *fila_receiver;

  if (uart_subscribe_int(&bit_no) != 0)
    return 1;

  irq_set = BIT(bit_no);
  int i = 0;
  //int j=0;
  //uint32_t leitura, aux_sender;
  unsigned char *aux = (unsigned char *)malloc(sizeof(unsigned char));
  unsigned char *aux1 = (unsigned char *)malloc(sizeof(unsigned char));
  unsigned char *aux2 = (unsigned char *)malloc(sizeof(unsigned char));
  unsigned char *aux3 = (unsigned char *)malloc(sizeof(unsigned char));
  unsigned char *aux4 = (unsigned char *)malloc(sizeof(unsigned char));
  unsigned char *aux5 = (unsigned char *)malloc(sizeof(unsigned char));
  unsigned char *aux6 = (unsigned char *)malloc(sizeof(unsigned char));
  unsigned char *aux7 = (unsigned char *)malloc(sizeof(unsigned char));
  uint32_t thr_sed;
  thr_sed = 'd';
  *aux = 'O';
  *aux1 = 'l';
  ...
  *aux6 = 'o';
  *aux7 = 'u';
  fila_push(get_transmitter(), aux);
  fila_push(get_transmitter(), aux1);
  fila_push(get_transmitter(), aux2);
  fila_push(get_transmitter(), aux3);
  fila_push(get_transmitter(), aux4);
  fila_push(get_transmitter(), aux5);
  fila_push(get_transmitter(), aux6);
  fila_push(get_transmitter(), aux7);

  if (serial_set(RACINIX_SERIAL_NUM_BITS, RACINIX_SERIAL_NUM_STOP_BITS, RACINIX_SERIAL_PARITY, RACINIX_SERIAL_BAUD_RATE))
  {
    return 1;
  }
